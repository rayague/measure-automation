...
| Go (external traces) | HotROD — Jaeger's official demo (4 services, MySQL, Redis) | [jaegertracing/jaeger](https://github.com/jaegertracing/jaeger/tree/main/examples/hotrod) | ✅ Validated (`mba ingest` on exported traces) | [below](#go-external-traces--jaeger-hotrod) |
...
## Go (external traces) — Jaeger HotROD

**Command:** `mba ingest hotrod-traces.json`
**Source:** [Jaeger's official HotROD demo](https://github.com/jaegertracing/jaeger/tree/main/examples/hotrod) — 4 Go microservices (frontend, customer, driver, route) with MySQL and Redis backends. Traces generated by driving 32 `/dispatch` requests, then exported from the Jaeger API (`/api/traces?service=frontend`).
**Date:** 2026-07-14 · MBA v0.9.4 · 32 traces, 1,264 spans.

### Why this matters

This is the first validation on trace data produced **entirely outside MBA's own pipeline** — instrumented by the Jaeger team, not by us. It exercised the pure-ingestion path (`mba ingest`) with zero deployment involvement.

### Result

| Metric | Value |
|---|---|
| Format detection | Jaeger, 100% confidence |
| Services attributed | 6/6 correct (customer, driver, frontend, mysql, redis-manual, route) |
| HTTP spans | 736 |
| DB spans | 32 (SQL carried in the legacy `sql.query` tag) |
| Table extraction | `customer` table correctly mapped to the `customer` service |
| SCOM | 0.0 for all services — **correct**: every HotROD service exposes a single route, and SCOM requires ≥2 endpoints per service |

### Honest status

**Validated — and it caught 3 real bugs** (fixed in v0.9.4, each with regression tests):

1. `mba ingest` collapsed multi-service trace files into the single majority service (the pipeline re-parsed the file with a global service-name override).
2. SQL in `sql.query` (OpenTracing legacy) and `db.query.text` (OTel semconv ≥1.26) tags was invisible; only `db.statement` was read.
3. Query strings were not stripped from endpoint keys (`GET /route?pickup=1,2` ≠ `GET /route?pickup=3,4` → 327 spurious endpoints).

The SCOM values themselves are trivially 0 and claim nothing about HotROD's design quality — reported as-is. Reproduce with:

```bash
docker network create hotrod-net
docker run -d --rm --net hotrod-net --name hotrod-jaeger -p 26686:16686 jaegertracing/all-in-one:1.60
docker run -d --rm --net hotrod-net --name hotrod -p 8089:8080 -e OTEL_EXPORTER_OTLP_ENDPOINT=http://hotrod-jaeger:4318 jaegertracing/example-hotrod:1.60 all
# browse http://localhost:8089 and click a few customers, or curl /dispatch?customer=123
curl "http://localhost:26686/api/traces?service=frontend&limit=60" -o hotrod-traces.json
mba ingest hotrod-traces.json
docker rm -f hotrod hotrod-jaeger && docker network rm hotrod-net
```
